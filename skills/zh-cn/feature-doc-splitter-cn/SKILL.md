...
description: "将粗略的功能需求初稿拆分为三份可直接用于开发的文档：总览文档、前端实现文档和后端实现文档。适用于需要阅读初版需求、结合现有代码、补齐业务流程、拆分前后端职责、强制总览文档包含 Mermaid 图、定义接口契约、沉淀验收标准，以及询问用户是否要为新增 UI 页面或区块预留可选 Pencil 设计稿占位的场景。"
---
...
使用这个技能，将早期功能需求初稿整理成一组三份可落地文档。输出必须结合真实代码结构，明确拆分前端和后端职责，并把不确定点显式写入假设或待确认事项。
